 ...
 * @author 200an
 */
public class OperadoresTSPHibrido implements OperadoresBases{
    
    private final ArrayList<GeneticoTSPHibrido> individuos;
    private int[] mascara;
    private int individuoEscogido;
    private int cantidadCiudades;

    public OperadoresTSPHibrido() {
    ...
    public int[] cruza(Object g1, Object g2) throws CloneNotSupportedException {
        ...
        GeneticoTSPHibrido g1_ = (GeneticoTSPHibrido) g1;
        GeneticoTSPHibrido g2_ = (GeneticoTSPHibrido) g2;
        
        //Obtenemos los genotipos(arreglo de bits) 
        //Pero clonamos la direccion del arreglo para evitar errores
        GeneticoTSPHibrido[] geneticos_aux = new GeneticoTSPHibrido[2];
        geneticos_aux[0] = (GeneticoTSPHibrido) g1_.clone();;
        geneticos_aux[1] = (GeneticoTSPHibrido) g2_.clone();
    ...
    public void muta(Object hijo, int max_Value) {
        ...
        GeneticoTSPHibrido hijoAux = (GeneticoTSPHibrido) hijo;
    ...
    public void evolucion(int poblacion, int generaciones, double probabilidad_cruza, double probabilidad_muta) throws CloneNotSupportedException {
        ...
        for(int numGeneracion = 0;numGeneracion < generaciones;numGeneracion++){
            ...
            GeneticoTSPHibrido[] nueva_poblacion = new GeneticoTSPHibrido[poblacion];//La siguiente generacion
            for(int individuo = 0; individuo < poblacion; individuo++){
                
                //Cruzamos a los padres
                //Pero hasta que la probabilidad de cruza sea mayor
                GeneticoTSPHibrido hijo = null;
                try {
                    hijo = new GeneticoTSPHibrido(new int[this.mascara.length]);
                ...
                do{
                    ...
                    GeneticoTSPHibrido padre = (GeneticoTSPHibrido) this.individuos.get(this.individuoEscogido).clone();
                    obtenerIndividuoAleatorio(this.individuos.size());
                    GeneticoTSPHibrido madre = (GeneticoTSPHibrido) this.individuos.get(this.individuoEscogido).clone();
                    ...
                    try {
                        hijo = new GeneticoTSPHibrido(cruza(padre,madre));
                    } catch (IOException ex) {
                        ex.printStackTrace();
                    }
            ...
            for(int numIndividuo = 0; numIndividuo < poblacion;numIndividuo++){
                this.individuos.set(numIndividuo, (GeneticoTSPHibrido) nueva_poblacion[numIndividuo].clone());
            }
        }
    }
    ...
    public Object getMejorIndividuo() throws CloneNotSupportedException {
        ...
        GeneticoTSPHibrido bestGenetico = (GeneticoTSPHibrido) this.individuos.get(0).clone();
        for(GeneticoTSPHibrido geneticoActual : this.individuos){
            if(geneticoActual.getFitness() < bestGenetico.getFitness()){
                bestGenetico = (GeneticoTSPHibrido) geneticoActual.clone();
            }
        }
        return bestGenetico;
    }
    ...
    public void generarPoblacionAleatoria(int tamPoblacion) {
        ...
        for(int i = 0; i < tamPoblacion; i++){
            try {
                this.individuos.add(new GeneticoTSPHibrido(obtenerGenotipoAleatorio(this.cantidadCiudades,this.cantidadCiudades)));
            } catch (IOException ex) {
                ex.printStackTrace();
            }
        }
